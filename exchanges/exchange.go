...
// support set
func (e *Base) SetFeatureDefaults(exch *config.ExchangeConfig) {
	if exch.Features == nil {
		e.RequiresConfigUpdate = true
		...
		if exch.SupportsAutoPairUpdates != nil {
			s.Supports.RESTCapabilities.AutoPairUpdates = *exch.SupportsAutoPairUpdates
		...
		if e.Features.Supports.RESTCapabilities.AutoPairUpdates != exch.Features.Supports.RESTCapabilities.AutoPairUpdates {
			exch.Features.Supports.RESTCapabilities.AutoPairUpdates = e.Features.Supports.RESTCapabilities.AutoPairUpdates
		...
		if e.Features.Supports.RESTCapabilities.AutoPairUpdates != exch.Features.Supports.RESTCapabilities.TickerBatching {
			exch.Features.Supports.RESTCapabilities.TickerBatching = e.Features.Supports.RESTCapabilities.AutoPairUpdates
			e.RequiresConfigUpdate = true
		}
...
func (e *Base) SetAPICredentialDefaults(exch *config.ExchangeConfig) {
...
func (e *Base) SetHTTPRateLimiter(exch *config.ExchangeConfig) {
	e.checkAndInitRequester()
...
func (e *Base) SetAssetTypes(exch *config.ExchangeConfig) {
	if exch.CurrencyPairs.AssetTypes == "" {
		exch.CurrencyPairs.AssetTypes = e.CurrencyPairs.AssetTypes.JoinToString(",")
		e.RequiresConfigUpdate = true
	} else {
		if exch.CurrencyPairs.AssetTypes != e.CurrencyPairs.AssetTypes.JoinToString(",") {
			exch.CurrencyPairs.AssetTypes = e.CurrencyPairs.AssetTypes.JoinToString(",")

			if e.IsAssetTypeSupported(assets.AssetTypeFutures) {
				exch.CurrencyPairs.Futures = new(config.CurrencyPairConfig)
			}
			e.RequiresConfigUpdate = true
		}
	}
...
func (e *Base) SetCurrencyPairFormat(exch *config.ExchangeConfig) {
	if e.CurrencyPairs.UseGlobalPairFormat {
		...
		if e.Features.Supports.Trading.Spot {
			if exch.CurrencyPairs.Spot == nil {
				exch.CurrencyPairs.Spot = new(config.CurrencyPairConfig)
			}
...
func (e *Base) SetupDefaults(exch *config.ExchangeConfig) error {
	...
	e.SetHTTPRateLimiter(exch)

	e.BaseCurrencies = common.SplitStrings(exch.BaseCurrencies, ",")
	if e.Features.Supports.Trading.Spot && exch.CurrencyPairs.Spot != nil {
	...
	if e.Features.Supports.Trading.Futures && exch.CurrencyPairs.Futures != nil {
	...
	e.SetCurrencyPairFormat(exch)
	e.SetAssetTypes(exch)
	e.SetFeatureDefaults(exch)
	e.SetAPIURL(exch)
	e.SetAPICredentialDefaults(exch)
	...
	if e.RequiresConfigUpdate {
		cfg := config.GetConfig()
		e.RequiresConfigUpdate = false
...
func (e *Base) UpdatePairs(exchangeProducts []string, assetType assets.AssetType, enabled, force bool) error {
	...
	if force || len(newPairs) > 0 || len(removedPairs) > 0 {
		...
		case assets.AssetTypeFutures:
			if enabled {
				exch.CurrencyPairs.Futures.Enabled = common.JoinStrings(products, ",")
				e.CurrencyPairs.Futures.Enabled = products
			} else {
				exch.CurrencyPairs.Futures.Available = common.JoinStrings(products, ",")
